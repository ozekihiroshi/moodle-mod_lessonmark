...
The asset check runs Moodle Grunt directly: Plugin CI 4.5.11 removes declared
build files before Moodle's ignorefiles task can inspect them. Empty placeholders
for the two declared generated files allow discovery; all AMD files and maps
must then be regenerated byte-for-byte. JavaScript, CSS and Behat feature lint
still run, and the committed build is restored even when verification fails.
